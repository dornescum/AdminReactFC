			...
			<Table striped responsive className="table-subscription">
				...
				{userCountySubscription.map((item) => {
					return <tr key={item.id}>
						<td>{item.id}</td>
						<td>{item.subscription === 'standard' ?
							<p className="standard">Standard</p> : item.subscription ||
						...
						<td>{item.status === 'Activ' ? <p className="pro">Activ</p> :
							<p className="inactive">Activeaza</p>}</td>
						<td>{item.extension ? <p className="pro">abonament activ</p> :
							<p className="inactive">Prelungire</p>}</td>
					</tr>;
				})
				}
